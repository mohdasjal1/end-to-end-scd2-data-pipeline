show streams;
select * from customer where customer_id = 73;
...
update customer set FIRST_NAME='Jessicaaaa', update_timestamp = current_timestamp()::timestamp_ntz where customer_id=72;
delete from customer where customer_id =73 ;


select * from customer_history where customer_id in (72,73,223136);
select METADATA$ACTION, METADATA$ISUPDATE, CUSTOMER_ID from customer_table_changes;
select * from customer_table_changes;
SELECT * FROM CUSTOMER_TABLE_CHANGES NO CONSUME);
